...
and ingests them as vector embeddings into Qdrant using FastEmbed (LOCAL/FREE).
...
def main() -> None:
    ...
    print(f"Domain: {domain_id} | Collection: {collection_name} (using FastEmbed LOCAL)")
    
    # Using FastEmbed: automatic local vectorization
    ...
    print(f"\nVectorizing and uploading {len(all_chunks)} chunks locally...")
    
    documents = [c["text"] for c in all_chunks]
    metadata = [{"source": c["source"], "domain_id": domain_id} for c in all_chunks]
    
    # qdrant.add will use fastembed locally to generate vectors and upsert them.
    # It also creates the collection if it doesn't exist with the right dimensions.
    qdrant.add(
    ...
    print(f"✅ {len(all_chunks)} documents indexed into '{collection_name}' (LOCAL).")
